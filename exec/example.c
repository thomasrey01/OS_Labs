...
void relocate(Elf64_Shdr *shdr, const Elf64_Sym *syms, const char *strings,
	      const char *src, char *dst)
{
	...
	for (j = 0; j < shdr->sh_size / sizeof(Elf64_Rel); j += 1) {
		const char *sym =
		    strings + syms[ELF32_R_SYM(rel[j].r_info)].st_name;
		switch (ELF32_R_TYPE(rel[j].r_info)) {
		case R_386_JMP_SLOT:
		case R_386_GLOB_DAT:
			*(Elf64_Word *)(dst + rel[j].r_offset) =
			    (Elf64_Word)resolve(sym);
			break;
		}
	}
}
...
void *image_load(char *elf_start, unsigned int size)
{
	...
	for (i = 0; i < hdr->e_phnum; ++i) {
		...
		if (!(phdr[i].p_flags & PF_W)) {
			// Read-only.
			mprotect((unsigned char *)taddr, phdr[i].p_memsz,
				 PROT_READ);
		}
	...
	return phdr[0].p_vaddr + exec;
	// return elf_start + hdr->e_entry;
	// return elf_start + 0xa069;
	// return entry;
