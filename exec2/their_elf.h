...
enum elf_proghdr_type {
	...
	ELF_PROG_DYNAMIC =
	    2, // this tells us about the dynamic linking that needs to be done
...
struct elf_header {
	uint32_t magic;	    // Value that indicates this is an ELF binary, must
			    // always match ELF_MAGIC
	byte __padding[12]; // Padding to align with a 16-byte boundary
	enum elf_hdr_type
	    type : 16;	  // The binary type - set to be 16 bits (2 bytes) long
	uint16_t machine; // We will only accept ELF_MACHINE_x64
	uint32_t
	    version; // We will only accept ELF_VERSION (the only version now)
	uint64_t entry; // Pointer to entrypoint of the program in "filespace"
	uint64_t proghdr_offset;   // Pointer to the program headers of this
				   // binary in "filespace"
	uint64_t secthdr_offset;   // Pointer to the section headers of this
				   // binary in "filespace"
	...
	uint16_t sectname_mapping; // The index of the section where the mapping
				   // between section index and name is located
} __attribute__((packed));

// The structure of the program header, located at proghdr_offset
struct elf_proghdr {
	enum elf_proghdr_type
	    type : 32; // The type of this program header - must be 4 bytes long
	enum elf_proghdr_flag
	    flags : 32; // Some flags for this program header - again 4 bytes
	uint64_t file_offset;	  // Pointer to the contents of this segment in
				  // "filespace"
	uint64_t virtual_address; // The preferred virtual memory address to
				  // load this segment - an absolute value for
				  // non-relocatable code, but relative for
				  // relocatable code
	uint64_t
	    physical_address; // For machines without page tables, the preferred
			      // physical memory address to load this segment
	uint64_t size_file;   // The size of this segment in "filespace"
	uint64_t size_memory; // The desired size of this segment in memory
			      // (might not equal size_file!)
	uint64_t alignment;   // The desired alignment of this segment
} __attribute__((packed));

// The structure of the section header, located at secthdr_offset
struct elf_secthdr {
	uint32_t name; // Pointer to a string with the name of this section in
		       // "filespace"
	enum elf_secthdr_type
	    type : 32;		  // The type of this section header - 4 bytes
	uint64_t flags;		  // Some flags
	uint64_t virtual_address; // The address this section is assmed to be at
	uint64_t file_offset;	  // Pointer to the contents of this section in
				  // "filespace"
	...
	uint64_t entry_size; // If a section describes a list of "entries", the
			     // size of each entry
} __attribute__((packed));

// The structure for the auxiliary elf vectors, located on the stack right after
